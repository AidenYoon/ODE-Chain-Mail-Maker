...
struct PrintingContext {
    ...
    void printChar (const char *name, const char *x=0);
    void printNum (const char *name, dReal x);
...
void PrintingContext::printNum (const char *name, dReal x)
{
    printReal (x);
    fprintf (file,"\n");
}

void PrintingContext::printChar (const char *name, const char *x)
{
    if (x) {
        fprintf (file,"%s = \"%s\",\n",name,x);
    }
